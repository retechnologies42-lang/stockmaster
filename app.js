...
var GAS_URL = "https://script.google.com/macros/s/AKfycbwzdtc53obxRBJzuP_omnUhSUD4ckUx62girJhgYWDcPSh0OMdmGaQN8U472wFnnA/exec";
...
function computeItemSalesState(item){
  if(!item)return{code:"work",label:"In Bearbeitung",cls:"ros-st-work",score:50,border:"ros-border-work"};
  var hasImg=!!((item.kaFotos&&item.kaFotos.length)||(item.fotos&&item.fotos.length));
  var hasVk=parseFloat(item.kaPreis||0)>0;
  var hasEk=parseFloat(item.einkaufspreis||0)>0;
  var hasPrice=hasVk||hasEk;
  var kaDone=String(item.kleinanzeigen||"").toLowerCase().indexOf("hochgeladen")>-1||item.kleinanzeigen==="ja";
  ...
  return{code:"work",label:"In Bearbeitung",cls:"ros-st-work",score:58,border:"ros-border-work"};
}
function estimateSalesChance(item){
...
function buildListingTitle(item){
  var nm=item.name||item.spiel||item.modell||"Artikel";
  var sys=item.system||item.typ_||"";
  var z=item.zustand||"";
  var parts=[nm];
...
function downloadItemPhotos(item){
  var imgs=(item.kaFotos&&item.kaFotos.length)?item.kaFotos:(item.fotos||[]);
  if(!imgs.length){toast("Keine Bilder vorhanden","err");return;}
  var nm=(item.name||item.spiel||item.modell||"artikel").replace(/[^a-z0-9_-]+/gi,"_").slice(0,40);
  function dlOne(ix){
    var a=document.createElement("a");
    a.href=imgs[ix];
    a.download=nm+"_"+(ix+1)+".jpg";
    a.click();
  }
  if(imgs.length===1){dlOne(0);toast("1 Bild wird geladen","ok");return;}
  if(typeof JSZip==="undefined"){
    toast("ZIP wird vorbereitet…","inf");
    var s=document.createElement("script");
    s.src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js";
    s.onload=function(){downloadItemPhotos(item);};
    s.onerror=function(){var i=0;function next(){if(i>=imgs.length){toast(imgs.length+" Bilder gestartet","ok");return;}dlOne(i++);setTimeout(next,350);}next();};
    document.head.appendChild(s);
    return;
  }
  var zip=new JSZip();
  var pending=imgs.length,done=0,failed=0;
  imgs.forEach(function(src,ix){
    try{
      var base=src.indexOf(",")>-1?src.split(",")[1]:src;
      zip.file(nm+"_"+(ix+1)+".jpg",base,{base64:true});
      done++;
    }catch(e){failed++;}
    pending--;
    if(pending===0){
      zip.generateAsync({type:"blob"}).then(function(blob){
        ...
        a.download=nm+"_fotos.zip";
        a.click();
        setTimeout(function(){URL.revokeObjectURL(a.href);},2000);
        toast(done+" Bilder als ZIP"+(failed?" ("+failed+" übersprungen)":""),"ok");
      },function(){toast("ZIP fehlgeschlagen","err");});
    }
  });
}
function renderDetailKAPrep(item){
  ensureResellerOSStyles();
  var host=document.getElementById("detail-ka-prep");
  if(!host){
    var body=document.querySelector("#detail-overlay .detail-body");
    if(!body)return;
    host=document.createElement("div");
    host.id="detail-ka-prep";
    var specs=document.getElementById("detail-specs");
    if(specs&&specs.parentNode)specs.parentNode.insertBefore(host,specs);
...
function renderIntakePhaseBar(step){
  ...
  if(!host){
    var stepper=document.getElementById("main-stepper");
    if(!stepper)return;
    host=document.createElement("div");
    host.id="ros-intake-phases";
    host.className="ros-intake-phases";
    var wrap=stepper.querySelector(".wrap")||stepper;
    var prog=wrap.querySelector(".prog-wrap");
...
var belegeCache=[];
window._realityFilter=window._realityFilter||"";
...
function renderHomeControlHub(){
  ...
  var noImg=stockItems.filter(function(i){return !((i.kaFotos&&i.kaFotos.length)||(i.fotos&&i.fotos.length));});
  ...
  var pipe=[
    {l:"Einkauf",on:"goTabFn('handel-panel');setHandelTab('einkauf')"},
    {l:"Aufnahme",on:"goTabFn('scan-panel');setMode('einlagern')"},
    {l:"Optimierung",on:"goTabFn('list-panel','all')"},
    {l:"Upload",on:"openUploadWizard('Kleinanzeigen')"},
    {l:"Verkauf",on:"openVerkaufForm()"},
    {l:"Versand",on:"goTabFn('handel-panel');setHandelTab('verkauf')"},
  ...
  var prioHtml=actions.map(function(a){return'<div class="hh-prio-item" onclick="'+a.c+'"><div class="i"><i class="bi '+a.ic+'"></i></div><div style="font-size:12px;color:#e6edf3;font-weight:600">'+esc(a.t)+'</div></div>';}).join("");
  var statTiles=[
...
function goTabFn(id,lfMode){if(restrictedActivationMode&&id!=="klausel-panel"){toast("Nur Klausel-Bereich verfügbar.","err");return;}document.querySelectorAll(".bottom-nav .bnav-btn").forEach(function(b){b.classList.toggle("on",b.dataset.tab===id);});document.querySelectorAll(".panel").forEach(function(p){p.classList.toggle("on",p.id===id);});if(id!=="handel-panel"){var fm=document.getElementById("handel-fab-menu");if(fm)fm.style.display="none";}if(lfMode){if(id==="list-panel")setLF(lfMode);else{lf=lfMode;renderList();}}if(window._pendingAnDash&&id==="analyse-panel"){var pd=window._pendingAnDash;window._pendingAnDash=null;if(pd.range)window._anDash.range=pd.range;if(pd.plattform!==undefined)window._anDash.plattform=pd.plattform;if(pd.status!==undefined)window._anDash.status=pd.status;if(pd.chartMode)window._anDash.chartMode=pd.chartMode;if(pd.homeKpi!==undefined)window._anDash.homeKpi=pd.homeKpi;}if(id==="list-panel"&&allItems.length===0)loadAll();if(id==="home-panel"){setGreeting();loadStats();ensureHomeControlHub();if(!allItems.length){loadAll();}else{buildKAProgress();buildWeekChart();updateMyStats();renderHomeControlHub();}}if(id==="analyse-panel"){renderAnalysePanel();}if(id==="sets-panel"){renderSetsPanel();}if(id==="belege-panel"){renderBelegePanel();}if(id==="klausel-panel"){renderKlauselPanel();}}
function loadBelege(){try{belegeCache=JSON.parse(localStorage.getItem("smp_belege")||"[]")||[];}catch(e){belegeCache=[];}if(!Array.isArray(belegeCache))belegeCache=[];}
function saveBelege(){try{localStorage.setItem("smp_belege",JSON.stringify(belegeCache||[]));}catch(e){}}
function ensureBelegePanelUI(){if(document.getElementById("belege-panel"))return;var panel=document.createElement("section");panel.id="belege-panel";panel.className="panel";panel.innerHTML='<div class="wrap"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px"><h3 style="margin:0;color:var(--acc)">Belege</h3><button class="btn btn-outline-secondary btn-sm" onclick="renderBelegePanel()">Aktualisieren</button></div><div class="card" style="margin-bottom:10px"><div class="card-body"><div class="row g-2"><div class="col-md-3"><select id="beleg-typ" class="fc"><option value="EK">EK</option><option value="VK">VK</option></select></div><div class="col-md-5"><input id="beleg-name" class="fc" placeholder="Produktname / Belegname"/></div><div class="col-md-4"><button class="btn btn-success w-100" onclick="pickBelegImage()"><i class="bi bi-image me-1"></i>Bild hochladen</button></div></div></div></div><div id="belege-list"></div></div>';document.body.appendChild(panel);}
function openBelegePanel(){ensureBelegePanelUI();goTabFn("belege-panel");renderBelegePanel();}
function pickBelegImage(){var n=(document.getElementById("beleg-name")||{value:""}).value.trim();if(!n){toast("Name fehlt.","err");return;}var typ=(document.getElementById("beleg-typ")||{value:"EK"}).value||"EK";var inp=document.getElementById("beleg-file-inp");if(!inp){inp=document.createElement("input");inp.type="file";inp.accept="image/*";inp.id="beleg-file-inp";inp.style.cssText="position:absolute;left:-9999px";document.body.appendChild(inp);}inp.onchange=function(){var f=this.files&&this.files[0];if(!f)return;var fr=new FileReader();fr.onload=function(){belegeCache.unshift({typ:typ,name:n,img:String(fr.result||""),datum:new Date().toLocaleString("de-DE")});saveBelege();renderBelegePanel();toast("Beleg gespeichert.","ok");};fr.readAsDataURL(f);this.value="";};inp.click();}
function deleteBeleg(ix){belegeCache.splice(ix,1);saveBelege();renderBelegePanel();}
function renderBelegePanel(){loadBelege();var out=document.getElementById("belege-list");if(!out)return;if(!belegeCache.length){out.innerHTML='<div class="empty"><i class="bi bi-inbox"></i><p>Keine Belege vorhanden.</p></div>';return;}out.innerHTML=belegeCache.map(function(b,ix){return'<div class="ic" style="display:grid;grid-template-columns:64px 1fr auto;gap:10px;align-items:center"><div style="width:64px;height:64px;border-radius:8px;overflow:hidden;border:1px solid #222">'+(b.img?'<img src="'+esc(b.img)+'" style="width:100%;height:100%;object-fit:cover"/>':'')+'</div><div><div style="font-size:13px;color:#fff;font-weight:700">'+esc((b.typ||"EK")+' · '+(b.name||""))+'</div><div style="font-size:10px;color:var(--w4)">'+esc(b.datum||"")+'</div></div><button class="btn btn-outline-danger btn-sm" onclick="deleteBeleg('+ix+')"><i class="bi bi-trash3"></i></button></div>';}).join("");}
...
function deleteSetInline(ix){
  ...
  var backup=(window._setsPanelData||[]).slice();
  window._setsPanelData=(window._setsPanelData||[]).filter(function(_,i){return i!==ix;});
  renderSetsPanel();
  requestDeleteSet(s,function(ok,msg){
    if(ok){toast("Set gelöscht.","ok");loadAll(true);}
    else{window._setsPanelData=backup;renderSetsPanel();toast("Fehler: "+(msg||"?"),"err");}
  });
}
function requestDeleteSet(setRow,done){
  var sid=encodeURIComponent(String(setRow.setId||setRow.rowIndex||""));
  if(window.fetch&&sid){
    fetch("/api/sets/"+sid,{method:"DELETE"}).then(function(r){
      if(r&&r.ok){done&&done(true);return;}
      gasGet("deleteSetBundle",{rowIndex:setRow.rowIndex},function(gr){done&&done(!!(gr&&gr.ok),gr&&gr.fehler);},function(e){done&&done(false,e);});
    }).catch(function(){
      gasGet("deleteSetBundle",{rowIndex:setRow.rowIndex},function(gr){done&&done(!!(gr&&gr.ok),gr&&gr.fehler);},function(e){done&&done(false,e);});
    });
  }else{
...
function renderInboundCategoryChooser(){
  ...
  cc.innerHTML=
    '<div class="inb-wrap">'+
      '<div class="inb-head"><div class="inb-title">Produktaufnahme starten</div><div class="inb-sub">Kategorie wählen — Daten werden für Kleinanzeigen & Verkauf vorbereitet</div></div>'+
      '<div class="inb-grid">'+
...
function renderDamageStepUI(){
  ...
  s4.innerHTML='<div class="step-title">MÄNGEL</div><div class="step-sub">Wähle den Zustand des Artikels.</div>'+
    ...
    '<div id="damage-photo-row" style="display:none;margin-top:10px"><label class="fl">Defekt-Fotos</label><div class="photo-zone"><div class="pz-top" onclick="triggerPhotoInput(\'gallery\',\'damage\');setTimeout(renderDamageInlinePhotos,120)"><i class="bi bi-camera-plus"></i><div class="pz-l">Fotos hinzufügen</div><div class="pz-s">Max. 12 Fotos</div></div><div class="pz-btns"><button class="pz-btn" onclick="triggerPhotoInput(\'cam\',\'damage\');setTimeout(renderDamageInlinePhotos,120)"><i class="bi bi-camera me-1"></i>KAMERA</button><button class="pz-btn" onclick="triggerPhotoInput(\'gallery\',\'damage\');setTimeout(renderDamageInlinePhotos,120)"><i class="bi bi-images me-1"></i>GALERIE</button></div></div><div id="damage-photo-grid" class="photo-thumbs"></div></div>';
  renderDamageInlinePhotos();
}
function renderDamageInlinePhotos(){
  var g=document.getElementById("damage-photo-grid");if(!g)return;
  g.innerHTML="";
  (damagePhotos||[]).forEach(function(p,i){
...
function updateMangelCTA(){
  ...
  else if(probChoice==="major"&&damagePhotos.length===0){ok=false;msg="Defekt-Bild erforderlich";}
...
function updateProgress(){
  ...
  document.getElementById("prog-label").textContent=(isEditMode?"✏️ Produktaufnahme · ":"Produktaufnahme · ")+stepCur+"/"+stepTotal+" – "+(nn[stepCur-1]||"");
...
function stepNext(){
  ...
  if(stepCur===4){
    ...
    if(probChoice==="major"&&damagePhotos.length===0){toast("Mindestens 1 Defekt-Foto bei relevantem Schaden erforderlich.","err");return;}
  }
  if(stepCur===5){
...
function triggerPhotoInput(mode){
  var useCam=mode==="cam";
  var target=(arguments.length>1&&arguments[1]==="damage")?"damage":"main";
  var id=(useCam?"f-photo-cam":"f-photo-gallery")+"-"+target;
  ...
  el.onchange=function(){
    ...
    for(var i=0;i<files.length;i++)processPhotoFile(files[i],target);
    this.value="";
  };
  el.click();
}
function processPhotoFile(file,target){if(!file)return;var arr=target==="damage"?damagePhotos:photos;if(file.size>15*1024*1024){toast("Max. 15 MB pro Foto.","err");return;}if(arr.length>=12){toast("Maximal 12 Bilder pro Kategorie.","err");return;}var name=(file.name||"foto.jpg").replace(/[^a-zA-Z0-9._-]/g,"_");var img=new Image(),url=URL.createObjectURL(file);img.onload=function(){URL.revokeObjectURL(url);var MAX=1200,w=img.width,h=img.height;if(w>MAX||h>MAX){if(w>h){h=Math.round(h*(MAX/w));w=MAX;}else{w=Math.round(w*(MAX/h));h=MAX;}}var canvas=document.createElement("canvas");canvas.width=w;canvas.height=h;var ctx=canvas.getContext("2d");ctx.fillStyle="#ffffff";ctx.fillRect(0,0,w,h);ctx.drawImage(img,0,0,w,h);var b64=canvas.toDataURL("image/jpeg",0.78);if(!b64||b64.indexOf("base64,")===-1){toast("Bild konnte nicht verarbeitet werden.","err");return;}arr.push({b64:b64,name:name});renderAllPhotos();renderDamageInlinePhotos();toast("Foto hinzugefügt ✅","ok",2000);};img.onerror=function(){toast("Bild konnte nicht geladen werden.","err");};img.src=url;}
...
function removePhoto(idx,target){var arr=target==="damage"?damagePhotos:photos;arr.splice(idx,1);renderAllPhotos();renderDamageInlinePhotos();}
...
function doSave(){
  ...
  if(effectiveMainCount<1)miss.push("Foto");
  if(probChoice==="major"&&effectiveDefektCount<1)miss.push("Defekt-Foto");
  ...
  if(!isEditMode&&dup&&!window._saveDupBypass){
...
function resetStepperState(){probChoice=null;photos=[];damagePhotos=[];["f-scanid","f-name","f-ma","f-prob-beschr","f-einkaufspreis"].forEach(function(id){var e=document.getElementById(id);if(e)e.value="";});var mw=document.getElementById("photo-main-wrap");if(mw)mw.innerHTML="";var pt=document.getElementById("photo-thumbs");if(pt)pt.innerHTML="";var pdr=document.getElementById("prob-descr-row");if(pdr)pdr.style.display="none";var phr=document.getElementById("damage-photo-row");if(phr)phr.style.display="none";// Fix 1: Barcode-Banner zurücksetzen
...
// ── LAGER ─────────────────────────────────────────────────────────
var _loadAllBusy=false,_loadAllTs=0,_loadAllCache=[],_loadAllOnDone=null;
var addToSetModalState={item:null,query:"",selectedSetId:""};
var setInfoOverlayState={item:null};
function loadAll(force){
  var onDone=(arguments.length>1&&typeof arguments[1]==="function")?arguments[1]:null;
  if(onDone)_loadAllOnDone=onDone;
  ...
  if(!force&&_loadAllCache.length>0&&(now-_loadAllTs)<15000){
    ...
    var od=_loadAllOnDone;_loadAllOnDone=null;if(od)try{od();}catch(e){}
    return;
  }
  _loadAllBusy=true;
  ...
  function pruneSoldItemsFromSets(){
    var idx={};(allItems||[]).forEach(function(i){var sid=String(i.scanId||"").trim();if(sid)idx[sid]=i;});
    var changed=false,removed=0;
    (sb||[]).forEach(function(setRow){
      var before=(setRow.items||[]).length;
      setRow.items=(setRow.items||[]).filter(function(it){
        var sid=String(it.scanId||"").trim();if(!sid)return true;
        var item=idx[sid];
        if(!item)return false;
        if(itemIsLagerSoldOut&&itemIsLagerSoldOut(item))return false;
        var st=String(item.status||"").toLowerCase();
        if(st==="verkauft"||st==="archiviert")return false;
        return true;
      });
      if(setRow.items.length!==before){
        changed=true;removed+=(before-setRow.items.length);
        gasPost("updateSetBundle",{rowIndex:setRow.rowIndex,name:setRow.name,plattform:setRow.plattform,budget:setRow.budget,zustand:setRow.zustand,items:setRow.items,notizen:setRow.notizen||"Auto-bereinigt (verkauft)"},function(){},function(){});
      }
    });
    if(changed&&removed>0){toast("Einige Artikel wurden entfernt, da sie bereits verkauft sind","inf",3800);}
  }
  function tryR(){
    done++;
    renderPartial();
    ...
    _loadAllBusy=false;
    syncIncompleteProductTasks();renderList();checkLongStorageItems();buildWeekChart();buildKAProgress();updateMyStats();
    var od2=_loadAllOnDone;_loadAllOnDone=null;if(od2)try{od2();}catch(e2){}
  }
...
function refreshInventoryWithLoading(done){
  var listBody=document.getElementById("list-body");
  if(listBody)listBody.innerHTML='<div class="empty"><span class="spin-b"></span><p>Lade…</p></div>';
...
function renderList(){
  ...
  var f=allItems.filter(function(i){
    if(i.type==="setbundle")return lf==="all";
...
function mkCard(item){
  ...
  if(item.zustand)tags.push(String(item.zustand));
  var setHintText="";
  var setRefs=[];
  var sid=String(item.scanId||"").trim();
  if(sid&&setMembershipByScanId[sid]&&setMembershipByScanId[sid].length){
    setRefs=(setMembershipByScanId[sid]||[]);
    var sn0=(setRefs[0]&&setRefs[0].name)||(setRefs[0]&&setRefs[0].setId)||"";
    if(setRefs.length===1)setHintText="Teil von Set: "+String(sn0||"Set");
    else setHintText="Teil von Set: "+String(sn0||"Set")+" (+"+(setRefs.length-1)+")";
  }
  ...
  var preview=(item.kaFotos&&item.kaFotos[0])?item.kaFotos[0]:((item.fotos&&item.fotos[0])?item.fotos[0]:"");
  var priceLabel=(item.kaPreis?String(item.kaPreis)+" €":(item.einkaufspreis?String(item.einkaufspreis)+" €":"VK offen"));
  var catLabel=tm[1];
  ...
  return '<div class="lager-card '+salesSt.border+saleCls+'" onclick="openDetail('+rIdx+')" style="cursor:pointer"><div class="lager-head"><div class="lager-title"'+titleStyle+'>'+esc(nm)+'</div>'+actions+'</div><div class="lager-card-row"><div class="lager-thumb">'+(preview?'<img src="'+esc(preview)+'" loading="lazy"/>':'<div style="font-size:20px;display:flex;align-items:center;justify-content:center;width:100%;height:100%">📦</div>')+'</div><div><div class="lager-price">'+esc(priceLabel)+'</div><div class="lager-sub">'+statusBadge+'</div><div class="ros-chance">Verkaufschance: <b>'+chance+'%</b></div><div class="lager-tags">'+tagsHtml+'</div>'+meta+(note?'<div style="font-size:11px;color:#9ca3af;margin-top:6px">'+esc(note)+'</div>':"")+(setHintText?'<div class="lager-sethint" onclick="event.stopPropagation();openSetInfoOverlay('+rIdx+')"><i class="bi bi-link-45deg"></i>'+esc(setHintText)+'</div>':'')+'</div></div></div>';
}
...
function confirmDeleteSetBundle(rIdx){
  var item=cardRegistry[rIdx];if(!item||item.type!=="setbundle")return;
  var btn=document.getElementById("del-modal-confirm");
  document.getElementById("del-modal-text").textContent="Set „"+(item.name||item.setId||"?")+"“ wirklich löschen?";
  if(btn){
...
function mkHandelCard(item,type,idx){
  ...
  if(isVk){
    var c=_vkCategoryLabel(item);
    if(c)meta+='<span class="wf-meta dim">Kategorie: '+esc(c)+'</span>';
  }
...
function _vkCategoryLabel(v){
  var ids=String(v.scanIds||"").split(/[,;\s]+/).map(function(x){return String(x||"").trim();}).filter(Boolean);
  if(!ids.length)return"";
  function fromSystem(it){
    var s=String(it.system||it.kategorie||"").toLowerCase();
    if(s.indexOf("nintendo")>-1||s.indexOf("switch")>-1||s.indexOf("wii")>-1)return"Nintendo";
    if(s.indexOf("playstation")>-1||/\bps[1-5]\b/.test(s))return"PlayStation";
    if(s.indexOf("xbox")>-1)return"Xbox";
    if(s.indexOf("wii")>-1)return"Wii";
    return"";
  }
  var map={konsole:"Konsole",spiel:"Spiel",controller:"Controller",handy:"Handy",pc:"PC"};
  for(var i=0;i<ids.length;i++){
    var it=(allItems||[]).find(function(x){return String(x.scanId||"").trim()===ids[i];});
    if(it){
      var sp=fromSystem(it);
      if(sp)return sp;
      if(map[it.type])return map[it.type];
    }
  }
  return"";
}
...
function openDetail(rIdx){
  ...
  detailItem=item;
  var nm=item.name||item.spiel||item.modell||item.geraet||"–";
  if(item.type==="setbundle"){
    document.getElementById("detail-header-title").textContent=nm;
    document.getElementById("detail-title").textContent=nm;
    var eb0=document.getElementById("detail-edit-btn");if(eb0)eb0.style.display="none";
    var pp0=document.getElementById("detail-price"),ps0=document.getElementById("detail-price-sub");
    if(pp0){pp0.textContent=item.einkaufspreis!=null&&item.einkaufspreis!==""?String(item.einkaufspreis)+" € (Summe EK)":"—";pp0.style.color="var(--acc)";}
    if(ps0)ps0.textContent="Set-ID: "+(item.setId||"—")+" · "+((item.items||[]).length)+" Artikel";
    var kab0=document.getElementById("detail-ka-badge");if(kab0)kab0.innerHTML="";
    var ptab=document.getElementById("detail-photo-tabs");if(ptab)ptab.style.display="none";
    var hero=document.getElementById("detail-hero");if(hero)hero.innerHTML='<div style="padding:24px;color:var(--w3);font-size:13px">Set-Bundle (Lager)</div>';
    var thumbs=document.getElementById("detail-thumbs");if(thumbs)thumbs.innerHTML="";
    var upWrap=document.getElementById("detail-upload-photos-wrap");if(upWrap)upWrap.style.display="none";
    var linksWrap=document.getElementById("detail-links-wrap");if(linksWrap)linksWrap.style.display="none";
    var dw=document.getElementById("detail-defekt-wrap");if(dw)dw.style.display="none";
    var nw=document.getElementById("detail-notes");if(nw)nw.style.display="none";
    var ul=(item.items||[]).map(function(it){return"<li>"+esc(it.name||it.scanId||"—")+"</li>";}).join("");
    document.getElementById("detail-specs").innerHTML='<div class="detail-spec"><span class="detail-spec-key">Artikel</span><span class="detail-spec-val"><ul style="margin:0;padding-left:18px">'+ul+"</ul></span></div>";
  ...
  if(eb){
    eb.style.display=item.type==="defekt"?"none":"inline-flex";
...
function scorePhotoSetForSale(photoArr,cb){
  var arr=photoArr||[];
  if(!arr.length){if(cb)cb({label:"Noch keine Fotos",cls:"ros-ph-bad",score:0});return;}
  var src=typeof arr[0]==="string"?arr[0]:(arr[0].b64||"");
  if(!src||src.indexOf("data:image")!==0){if(cb)cb({label:"Bilder vorhanden",cls:"ros-ph-mid",score:58});return;}
  var img=new Image();
  img.onload=function(){
    var c=document.createElement("canvas"),w=Math.min(140,img.width||140),h=Math.max(1,Math.round((img.height||140)*(w/(img.width||140))));
...
function showPhotoGuide(type){
  ...
  box.innerHTML='<div class="photo-guide-title"><i class="bi bi-camera-video" style="font-size:14px"></i>Kleinanzeigen Foto-Guide</div>'+
    tips.map(function(g,i){return'<div class="photo-guide-item"><div class="photo-guide-num">'+(i+1)+'</div><span>'+esc(g)+'</span></div>';}).join("")+
    '<div id="photo-score-box" class="ros-ph-score ros-ph-mid">Bildbewertung wird berechnet…</div>';
  scorePhotoSetForSale(photos,function(sc){
    var el=document.getElementById("photo-score-box");if(!el)return;
    el.className="ros-ph-score "+sc.cls;
    el.textContent=(sc.score>=80?"🟢 ":"")+(sc.score>=60&&sc.score<80?"🟡 ":"")+(sc.score<60?"🔴 ":"")+sc.label;
  });
}
...
function renderSetBuilderFlow(){
  ...
  loadSetChats();
  if(setBuilderStep===1&&(!allItems||!allItems.length)){
    if(!_loadAllBusy){loadAll(true,function(){renderSetBuilderFlow();});}
    body.innerHTML='<div class="empty" style="padding:20px"><span class="spin-b"></span><p style="color:var(--w3);margin-top:10px">Lager wird geladen…</p></div>';
    return;
  }
  if(setBuilderStep===0){
  ...
  var pool=_setEligibleItems();
  var stockList=pool.slice(0,120).map(function(i){
    var id=String(i.rowIndex||i.scanId||"");
    var nm=i.name||i.spiel||i.modell||i.scanId||"Unbekannt";
...
function sbBuildFromSelection(){
  var ids=[].slice.call(document.querySelectorAll("#sb-stock-list input[type='checkbox']:checked")).map(function(x){return x.value;});
  var pool=_setEligibleItems();
  var picks=pool.filter(function(i){return ids.indexOf(String(i.rowIndex||i.scanId||""))>-1;});
  if(!picks.length){toast("Bitte Artikel auswählen.","err");return;}
  var total=picks.reduce(function(s,i){return s+(parseFloat(i.einkaufspreis||0)||0);},0);
  var sumEinzel=Math.round(picks.reduce(function(s,i){return s+_sbSaleVal(i);},0)*100)/100;
  var fastMin=Math.round(sumEinzel*0.80*100)/100,fastMax=Math.round(sumEinzel*0.90*100)/100;
  var maxMin=Math.round(sumEinzel*0.95*100)/100,maxMax=Math.round(sumEinzel*1.00*100)/100;
  setBuilderDraft={name:"Bundle Set",plattform:"Kleinanzeigen",budget:0,zustand:"Gebraucht",items:picks.map(function(i){return{typ:i.type,name:i.name||i.spiel||i.modell||i.scanId,scanId:i.scanId||"",ek:parseFloat(i.einkaufspreis||0)||0,vk:_sbSaleVal(i),setId:i.setId||"",setName:i.setName||""};}),total:Math.round(total*100)/100,sumEinzel:sumEinzel,priceFastMin:fastMin,priceFastMax:fastMax,priceMaxMin:maxMin,priceMaxMax:maxMax};
...
function _sbSaleVal(it){
  var v=parseFloat(it.verkauf||it.vk||it.kaPreis||it.verkaufspreis||0);
  if(!isNaN(v)&&v>0)return v;
  var ek=parseFloat(it.ek||it.einkaufspreis||0)||0;
  return ek>0?ek*1.45:0;
}
function sbBuildSetKaAnalyse(draft){
  var items=draft.items||[],cnt=items.length;
  var salesVals=[];
  for(var i=0;i<cnt;i++){var sv=_sbSaleVal(items[i]);if(sv>0)salesVals.push(sv);}
  if(cnt<1||!salesVals.length){
    return{marktwert:{min:0,max:0},preise:{start:{min:0,max:0},verkauf:{min:0,max:0},schnell:{min:0,max:0}},nachfrage:{score:0,begruendung:"Keine verwertbaren Verkaufswerte."},optimierung:["Set aus Lager wählen oder Verkaufswerte ergänzen."]};
  }
  var avg=salesVals.reduce(function(s,v){return s+v;},0)/salesVals.length;
  var span=Math.max(4,avg*0.12);
  var markt=_sbPair(avg-span,avg+span);
  var preise={
    start:_sbPair(markt.max*1.03,markt.max*1.12),
    verkauf:_sbPair(markt.min*0.98,markt.max*1.02),
    schnell:_sbPair(markt.min*0.84,markt.min*0.93)
  };
  ...
  var hasSetRef=items.filter(function(it){return String(it.setId||it.setName||"").trim();}).length>0;
  var score=Math.max(1,Math.min(10,Math.round(4+(maxC/cnt)*5-(mix?1:0)+(cnt>=4?1:0)+(hasSetRef?1:0))));
  var demandTag=score>=8?"stark":(score>=5?"mittel":"niedrig");
  var beg=cnt+" Artikel · "+distinct+" Typ(en) · Nachfrage "+demandTag+" · Kombination "+(maxC===cnt?"fokussiert":"gemischt")+".";
  ...
  var opt;
  if(onlySpiele)opt="Nur Spiele-Set: Konsole ergänzen erhöht Abschlussquote.";
  else if(hasKonsole&&hasSpiele)opt="Konsole + Spiele passt gut zum aktuellen Nachfrageprofil.";
  else if(hasKonsole&&!hasSpiele)opt="Konsole ohne Spiele: 1-2 passende Titel als Hebel ergänzen.";
  else opt="Kombination aus "+cnt+" Teilen: Preis eng an der Verkaufs-Spanne halten.";
...
function sbRenderSetAnalyseCardHtml(data){
  var m=data.marktwert||{},p=data.preise||{},nf=data.nachfrage||{},op=(data.optimierung||[]).slice(0,1);
  var fmt=function(lo,hi){return Math.round(Number(lo)||0)+" – "+Math.round(Number(hi)||0)+" €";};
  var lines='<div class="sb-ka-line sb-ka-line-tight">Markt: '+fmt(m.min,m.max)+"</div>"
    +'<div class="sb-ka-line sb-ka-line-tight">Start (VB): '+fmt((p.start||{}).min,(p.start||{}).max)+"</div>"
...
function buildSetWithAI(onDone){
  ...
  setBuilderDraft={name:setTitle,plattform:plat,budget:budget,zustand:cond,items:picks.map(function(i){return{typ:i.type,name:i.name||i.spiel||i.modell||i.scanId,scanId:i.scanId||"",ek:ek(i),vk:_sbSaleVal(i),setId:i.setId||"",setName:i.setName||""};}),total:Math.round(total*100)/100,priceFast:"",priceMax:""};
  var sumEinzel=Math.round((picks.reduce(function(s,i){return s+_sbSaleVal(i);},0))*100)/100;
...
function renderSetManager(body){
  gasGet("getSetBundles",{},function(r){
    ...
    var idx={};(_setEligibleItems()||[]).forEach(function(i){var sid=String(i.scanId||"").trim();if(sid)idx[sid]=1;});
    var removed=0;
    setsCache=setsCache.map(function(s){
      var items=(s.items||[]).filter(function(it){var sid=String(it.scanId||"").trim();if(!sid)return true;var keep=!!idx[sid];if(!keep)removed++;return keep;});
      return Object.assign({},s,{items:items});
    });
    if(removed>0)toast("Einige Artikel wurden entfernt, da sie bereits verkauft sind","inf",3600);
...
function deleteSetFromManager(ix){
  var s=setsCache[ix];if(!s)return;
  if(!confirm("Set löschen?"))return;
  var backup=setsCache.slice();
  setsCache=setsCache.filter(function(_,i){return i!==ix;});
  var body=document.getElementById("sb-flow-body");if(body)renderSetManager(body);
  requestDeleteSet(s,function(ok,msg){
    if(ok){toast("Set gelöscht.","ok");setBuilderStep=99;renderSetBuilderFlow();loadAll(true);}
    else{setsCache=backup;toast("Fehler: "+(msg||"?"),"err");if(body)renderSetManager(body);}
  });
}
...
window.addEventListener("load",function(){ensureResellerOSStyles();
...
function _buildAnInsights(vkData,ctx){
  ...
  if(window._anDash.range==="month"){
    ...
    if(prevM>0&&curM<prevM*0.85)out.push({icon:"📉",text:"Marge unter Vormonat — Preise & EK jetzt optimieren →",cls:"an-in-warn"});
  }
  var neg=vkData.filter(function(v){return parseFloat(v.marge||0)<0;}).length;
  if(neg>0)out.push({icon:"💡",text:neg+" Verkauf"+(neg>1?"e":"")+" mit Verlust — Preise jetzt prüfen →",cls:"an-in-neg",issue:"negative_margin"});
  if(!out.length)out.push({icon:"✓",text:"Alles im grünen Bereich — weiter verkaufen.",cls:"an-in-ok",issue:""});
  var el=document.getElementById("an-insights-list");
  if(el){
    el.innerHTML=out.map(function(i){return'<div class="an-insight '+i.cls+'" '+(i.issue?'onclick="openRealityCheckIssue(\''+i.issue+'\')" style="cursor:pointer"':'')+'><span class="an-ins-ic">'+i.icon+'</span><p>'+i.text+"</p></div>";}).join("");
  }
}
function openRealityCheckIssue(issue){
  if(!issue)return;
  window._realityFilter=(issue==="missing_ek"||issue==="missing_vk")?issue:"";
  ...
  renderList();
  if(issue==="missing_ek"){
    var hit=(allItems||[]).find(function(i){return i.type!=="setbundle"&&i.type!=="defekt"&&!(parseFloat(i.einkaufspreis||0)>0);});
    if(hit)openEditStepperFromItem(hit);
  }else if(issue==="missing_vk"){
    var hitVk=(allItems||[]).find(function(i){return i.type!=="setbundle"&&i.type!=="defekt"&&!(parseFloat(i.kaPreis||0)>0);});
    if(hitVk)openEditStepperFromItem(hitVk);
  }
}
