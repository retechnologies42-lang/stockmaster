var GAS_URL = "https://script.google.com/macros/s/AKfycbzbeKaMteZUco7Unf3zAlH4CXMHuC1xcggjC5CHwC5o0I-gu069MpeR6LMsGuKwPg/exec";
...
function doSave(){
  ...
  gasPost(fn,d,
    function(r){setBL(btn,false,orig);if(r&&r.ok){toast(r.msg,"ok");addNotification(isEditMode?"✏️ Aktualisiert":"✅ Eingelagert",r.msg,"info");allItems=[];loadStats();isEditMode=false;editingItem=null;setTimeout(function(){loadAll();resetFlow();goTabFn("list-panel");},700);}else{toast("Fehler: "+(r?r.fehler:"?"),"err");}},
...
function openEinkaufForm(item) {
  ...
  // Fill fields
  sv2("ek-produkte", item ? item.produkte : "");
  sv2("ek-kunde", item ? item.kunde : "");
  sv2("ek-preis", item ? item.preis : "");
  sv2("ek-plattform", item ? item.plattform : "Kleinanzeigen");
  sv2("ek-zimmer", item ? item.zimmer : "");
  sv2("ek-adresse", item ? item.adresse : "");
  sv2("ek-sende", item ? item.sendenummer : "");
  sv2("ek-vdl", item ? item.versanddienstleister : "");
  sv2("ek-abholung", item ? item.abholung : "NEIN");
  sv2("ek-status", item ? item.status : "Bestellt");
  sv2("ek-lieferstatus", item ? item.lieferstatus : "Ausstehend");
  sv2("ek-ma", item ? item.mitarbeiter : emp);
  sv2("ek-hinweise", item ? item.hinweise : "");
  sv2("ek-zustand", "");
  // Highlight status
  if(item && item.status) selEKStatus(item.status);
  // Versand/Abholung
  var versandType = (item && item.abholung === "JA") ? "Abholung" : "Versand";
  selEKVersand(versandType);
  // Storno section
...
function _renderEKStep() {
  for(var i=1; i<=ekTotalSteps; i++) {
    var el = document.getElementById("eks-"+i);
    if(el) el.style.display = (i===ekStep ? "block" : "none");
  }
  var pct = Math.round((ekStep/ekTotalSteps)*100);
  var pb = document.getElementById("ek-prog"); if(pb) pb.style.width = pct+"%";
  var sl = document.getElementById("ek-step-lbl"); if(sl) sl.textContent = "STEP "+ekStep+"/"+ekTotalSteps;
  var snames = ["ARTIKEL","VERSAND","WARENEINGANG","ABSCHLUSS"];
  ...
  var nb = document.getElementById("ek-next-btn"); if(nb) nb.style.display = (ekStep<ekTotalSteps?"inline-flex":"none");
  var sb = document.getElementById("ek-save-btn"); if(sb) sb.style.display = (ekStep===ekTotalSteps?"inline-flex":"none");
  if(ekStep===3) _buildEKChecklist();
  if(ekStep===4) _buildEKSummary();
}
...
function ekStepNav(dir) {
  ...
  if(dir > 0) {
    if(ekStep===1) {
      if(!gv("ek-produkte").trim()) { showEKDiag("Bitte Produkt(e) eingeben."); return; }
      if(!gv("ek-kunde").trim()) { showEKDiag("Bitte Lieferant eingeben."); return; }
      if(!gv("ek-preis")) { showEKDiag("Bitte Preis eingeben."); return; }
    }
    if(ekStep===2) {
      if(!gv("ek-status")) { showEKDiag("Bitte Status wählen."); return; }
    }
  }
...
function saveEKForm() {
  var d = document.getElementById("ek-diag"); if(d) d.style.display="none";
  var data = {
    produkte: gv("ek-produkte"), kunde: gv("ek-kunde"), preis: gv("ek-preis"),
    plattform: gv("ek-plattform"), zimmer: gv("ek-zimmer"), adresse: gv("ek-adresse"),
    sendenummer: gv("ek-sende"), versanddienstleister: gv("ek-vdl"),
    abholung: gv("ek-abholung"), status: gv("ek-status"),
    lieferstatus: gv("ek-lieferstatus"), mitarbeiter: gv("ek-ma")||emp,
    hinweise: gv("ek-hinweise"), warentyp: "Gebrauchtware"
  };
  ...
  if(editEinkaufItem) {
    ...
    gasPost("updateEinkauf", data, function(r){
      setBL(btn,false);
      if(r&&r.ok){ toast(r.msg,"ok"); closeEKModal(); loadHandel(); }
      else { showEKDiag(r?r.fehler:"Fehler"); }
...
// ================================================================
// EINKAUF CHECK FLOW (Scan Panel)
// ================================================================
var ekCheckStep = 1; // 1=select einkauf, 2=checklist+scan
var ekCheckItem = null; // selected Einkauf
var ekCheckList = []; // checklist items [{name, scanId, checked, rowIndex}]
...
function _loadEKCheckList() {
  ...
  if(!listEl) return;
  listEl.innerHTML = '<div style="text-align:center;padding:16px;font-family:monospace;font-size:10px;color:var(--w4)"><span class="spin-b"></span> LADE EINKÄUFE...</div>';
  gasGet("getAllEinkauf", {}, function(r){
    if(!r||!r.ok){ listEl.innerHTML='<div class="empty"><i class="bi bi-inbox"></i><p>KEINE EINKÄUFE</p></div>'; return; }
    ...
    pending.forEach(function(ek){
      var item = document.createElement("div");
      item.className = "ic";
      item.style.cssText = "cursor:pointer;margin-bottom:7px";
      var statusColor = {Bestellt:"var(--col-y)",Bezahlt:"var(--col-b)",Versendet:"var(--col-b)",Angekommen:"var(--acc)"}[ek.status]||"var(--w4)";
      item.innerHTML = '<div class="ic-top"><div class="ic-name">'+esc(ek.produkte||"–")+'</div>'
        +'<span style="font-size:9px;font-weight:700;color:'+statusColor+';font-family:monospace">'+esc(ek.status||"–")+'</span></div>'
        +'<div class="chips">'
        +(ek.zimmer?'<span class="chip"><b>📍 '+esc(ek.zimmer)+'</b></span>':"")
        +(ek.kunde?'<span class="chip">'+esc(ek.kunde)+'</span>':"")
        +(ek.preis?'<span class="chip" style="font-family:monospace">'+esc(ek.preis)+'€</span>':"")
        +'</div>';
      item.onclick = (function(e){ return function(){ selectEKCheckItem(e); }; })(ek);
...
function selectEKCheckItem(ek) {
  ekCheckItem = ek;
  ekCheckStep = 2;
  // Parse products into checklist
  var lines = (ek.produkte||"").split(",").map(function(p){return p.trim();}).filter(Boolean);
  ekCheckList = lines.map(function(name){ return {name:name, scanned:false, scanId:""}; });
  _renderEKCheckPanel();
}

function _renderEKCheckPanel() {
  var step1 = document.getElementById("ek-check-step1");
  var step2 = document.getElementById("ek-check-step2");
  if(step1) step1.style.display = ekCheckStep===1 ? "block" : "none";
  if(step2) step2.style.display = ekCheckStep===2 ? "block" : "none";
  if(ekCheckStep!==2 || !ekCheckItem) return;

  var infoEl = document.getElementById("ek-check-info");
  if(infoEl){
    infoEl.innerHTML = '<div style="font-size:10px;font-weight:700;color:var(--acc);font-family:monospace;letter-spacing:.5px;margin-bottom:4px">EINKAUF // '+esc(ekCheckItem.zimmer||"KEIN ZIMMER")+'</div>'
      +'<div style="font-size:13px;font-weight:700;color:var(--w1)">'+esc(ekCheckItem.produkte||"–")+'</div>'
      +'<div style="font-size:10px;color:var(--w3);margin-top:2px;font-family:monospace">'+esc(ekCheckItem.kunde||"–")+(ekCheckItem.preis?" · "+ekCheckItem.preis+"€":"")+'</div>';
  }

  var done = ekCheckList.filter(function(i){return i.scanned;}).length;
  var total = ekCheckList.length;
  var prog = document.getElementById("ek-check-progress");
  if(prog){
    prog.innerHTML = '<div style="display:flex;justify-content:space-between;margin-bottom:4px">'
      +'<span style="font-size:9px;font-family:monospace;color:var(--w4);letter-spacing:.5px">FORTSCHRITT</span>'
      +'<span style="font-size:9px;font-family:monospace;color:var(--acc)">'+done+'/'+total+'</span></div>'
      +'<div style="background:var(--e1);border-radius:99px;height:3px;overflow:hidden">'
      +'<div style="height:100%;background:var(--acc);border-radius:99px;width:'+(total>0?Math.round(done/total*100):0)+'%;transition:width .3s"></div></div>';
  }

  var listEl = document.getElementById("ek-check-items");
  if(!listEl) return;
  listEl.innerHTML = "";
  ekCheckList.forEach(function(item, idx){
    var row = document.createElement("div");
    row.style.cssText = "display:flex;align-items:center;gap:10px;padding:10px 0;border-bottom:1px solid var(--e1)";
    var checkIcon = item.scanned
      ? '<div style="width:22px;height:22px;border-radius:50%;background:var(--acc);display:flex;align-items:center;justify-content:center;flex-shrink:0"><i class="bi bi-check" style="color:#000;font-size:13px"></i></div>'
      : '<div style="width:22px;height:22px;border-radius:50%;border:1.5px solid var(--e2);flex-shrink:0"></div>';
    row.innerHTML = checkIcon
      + '<div style="flex:1">'
      + '<div style="font-size:13px;font-weight:600;color:'+(item.scanned?"var(--acc)":"var(--w1)")+'">'+esc(item.name)+'</div>'
      + (item.scanned && item.scanId ? '<div style="font-size:10px;color:var(--w4);font-family:monospace">'+esc(item.scanId)+'</div>' : '')
      + '</div>'
      + (!item.scanned ? '<button onclick="startEKItemScan('+idx+')" style="background:var(--acc);color:#000;border:none;border-radius:4px;padding:5px 10px;font-family:monospace;font-size:9px;font-weight:700;cursor:pointer;letter-spacing:.5px">SCAN</button>' : '');
    listEl.appendChild(row);
  });

  // Complete button
  var allDone = ekCheckList.every(function(i){return i.scanned;});
  var completeBtn = document.getElementById("ek-check-complete");
  if(completeBtn){
    completeBtn.style.display = allDone ? "block" : "none";
  }
}

function startEKItemScan(itemIdx) {
  var wrapEl = document.getElementById("ek-check-scan-wrap");
  if(!wrapEl) return;
  wrapEl.style.display = "block";
  wrapEl.setAttribute("data-target-idx", itemIdx);
  var video = document.getElementById("ek-check-scan-video");
  navigator.mediaDevices.getUserMedia({video:{deviceId:firstCamDeviceId?{exact:firstCamDeviceId}:undefined,facingMode:"environment"}})
    .then(function(stream){
      ekCheckScanStream = stream;
      ekCheckScanActive = true;
      video.srcObject = stream;
      _ekCheckScanLoop(itemIdx);
    }).catch(function(e){ toast("Kamera: "+e.message,"err"); wrapEl.style.display="none"; });
}
...
function _ekCheckScanLoop(itemIdx) {
  if(!ekCheckScanActive) return;
  var video = document.getElementById("ek-check-scan-video");
  var canvas = document.getElementById("ek-check-scan-canvas");
  if(!video||!canvas||video.readyState<2){ requestAnimationFrame(function(){_ekCheckScanLoop(itemIdx);}); return; }
  canvas.width=video.videoWidth; canvas.height=video.videoHeight;
  var ctx=canvas.getContext("2d"); ctx.drawImage(video,0,0);
  try {
    var bd=new BarcodeDetector({formats:["ean_13","ean_8","code_128","upc_a","upc_e","qr_code"]});
    bd.detect(canvas).then(function(codes){
      if(codes.length>0 && ekCheckScanActive){
        var bc = codes[0].rawValue;
        stopEKCheckScanner();
        _ekCheckMarkItem(itemIdx, bc);
      } else { requestAnimationFrame(function(){_ekCheckScanLoop(itemIdx);}); }
    }).catch(function(){ requestAnimationFrame(function(){_ekCheckScanLoop(itemIdx);}); });
  } catch(e){ requestAnimationFrame(function(){_ekCheckScanLoop(itemIdx);}); }
}

function ekCheckManualScan(itemIdx) {
  var input = document.getElementById("ek-check-manual-input");
  var val = input ? input.value.trim() : "";
  if(!val){ toast("Bitte Barcode eingeben","err"); return; }
  _ekCheckMarkItem(itemIdx, val);
  if(input) input.value = "";
}

function _ekCheckMarkItem(itemIdx, scanId) {
  if(itemIdx < 0 || itemIdx >= ekCheckList.length) return;
  ekCheckList[itemIdx].scanned = true;
  ekCheckList[itemIdx].scanId = scanId;
  toast(esc(ekCheckList[itemIdx].name)+" ✓ gescannt","ok",2000);
  // Einlagern starten für diesen Artikel
  var item = ekCheckList[itemIdx];
  // Set scan ID for next einlagern
  sv2("f-scanid", scanId);
  // Store context for after einlagern
  window._ekCheckContext = {itemIdx:itemIdx, scanId:scanId, itemName:item.name};
  _renderEKCheckPanel();
}

function completeEKCheck() {
  if(!ekCheckItem) return;
  if(!ekCheckList.every(function(i){return i.scanned;})){
    if(!confirm("Noch nicht alle Artikel gescannt. Trotzdem abschließen?")) return;
  }
  var btn = document.getElementById("ek-check-complete-btn");
  if(btn) setBL(btn, true);
  gasPost("updateEinkauf", {rowIndex: ekCheckItem.rowIndex, status:"Abgeschlossen", lieferstatus:"Zugestellt"}, function(r){
    if(btn) setBL(btn, false);
    if(r&&r.ok){
      toast("Einkauf abgeschlossen! ✅","ok",3000);
      closeEKCheck();
      loadHandel();
    } else { toast("Fehler: "+(r?r.fehler:"?"),"err"); }
...
function openRTModal(item, prefillVerkauf) {
  var editRTItem_local = item || null;
  window.editRTItem = editRTItem_local;
  rtStep = 1;
  ...
  sv2("rt-scanid", item ? item.scanId : "");
  ...
  sv2("rt-ma", item ? item.mitarbeiter : emp);
...
function rtStepNav(dir) {
  ...
  if(dir>0){
    if(rtStep===1 && !gv("rt-produkt").trim()){
      var dg=document.getElementById("rt-diag"); dg.className="diag derr"; dg.textContent="Bitte Produkt eingeben."; dg.style.display="block"; return;
    }
    if(rtStep===2 && !gv("rt-grund")){
      var dg=document.getElementById("rt-diag"); dg.className="diag derr"; dg.textContent="Bitte Grund wählen."; dg.style.display="block"; return;
    }
  }
...
function _buildRTSummary() {
  ...
  el.innerHTML = "PRODUKT: "+(gv("rt-produkt")||"–")+"<br>"
...
function saveRTForm() {
  ...
  if(!grund){var d=document.getElementById("rt-diag");d.className="diag derr";d.textContent="Bitte Grund wählen.";d.style.display="block";return;}
  var data = {
    produkt: gv("rt-produkt"), kunde: gv("rt-kunde"), scanId: gv("rt-scanid"),
    grund: grund, status: gv("rt-status"), erstattung: gv("rt-erstattung"),
    hinweise: gv("rt-hinweise"), mitarbeiter: gv("rt-ma")||emp,
  ...
  if(window.editRTItem){
    ...
    gasPost("updateRetoure", data, function(r){
      setBL(btn,false);
      if(r&&r.ok){toast(r.msg,"ok");closeRTModal();}
      else{var d=document.getElementById("rt-diag");d.className="diag derr";d.textContent=r?r.fehler:"Fehler";d.style.display="block";}
    },function(e){setBL(btn,false);toast("Fehler: "+e,"err");});
  } else {
    gasPost("saveRetoure", data, function(r){
      setBL(btn,false);
      if(r&&r.ok){toast(r.msg,"ok");closeRTModal();addNotification("⚠️ Reklamation",data.produkt+" – "+grund,"alert");}
      else{var d=document.getElementById("rt-diag");d.className="diag derr";d.textContent=r?r.fehler:"Fehler";d.style.display="block";}
    },function(e){setBL(btn,false);toast("Fehler: "+e,"err");});
  }
}
...
function selRTGrund(grund) {
  ...
  ["defekt","falsch","beschaedigt","nonfunc","sonstiges"].forEach(function(id){
    var el=document.getElementById("rtg-"+id);
    if(el) el.className="cbtn";
  });
  var map={"Defekt erhalten":"defekt","Falsches Produkt":"falsch",
